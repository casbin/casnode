...
func TestRemoveSyncGitterData(t *testing.T) {
	...
	for _, node := range nodes {
		...
		_, err = adapter.Engine.Query("DELETE t.*,r.* FROM topic as t LEFT JOIN reply as r ON t.id = r.topic_id WHERE t.gitter_message_id is not null AND t.node_id = ?", node.Id)
		if err != nil {
			panic(err)
		}
